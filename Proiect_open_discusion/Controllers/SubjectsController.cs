...
namespace Proiect_open_discusion.Controllers
{
    ...
    public class SubjectsController : Controller
    {
        ...
        public ActionResult Index(string SearchText, string sortOrder, int id, int pg = 1)
        {
            var subjects = db.Subjects.Where(subject => subject.CategoryId == id).ToList();
            ViewData["Current"] = sortOrder;
            ViewData["NameOrder"] = String.IsNullOrEmpty(sortOrder) ? "name_desc" : "";

            var category = db.Categories.Find(id);
            ViewBag.Category = category;

            List<Subject> subjs = db.Subjects.Where(subject => subject.CategoryId == id).Include("User").ToList();
        ...
        public ActionResult Edit(int id, Subject updatedSubject)
        {
            ...
            if (ModelState.IsValid)
            {
                if (subject.UserId.ToString() == userManager.GetUserId(User) || User.IsInRole("Moderator") || User.IsInRole("Administrator"))
                {
                    ...
                    return RedirectToAction("Index", new { id = updatedSubject.CategoryId });
                }
                else
                {
                    return RedirectToAction("Index", new { id = updatedSubject.CategoryId });
